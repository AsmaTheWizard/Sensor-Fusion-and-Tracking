...
def detect_objects(input_bev_maps, model, configs):
    ...
    for item in detections:
        id_, bev_x_, bev_y_, z_, h_, bev_w_, bev_l_, yaw_ = item
        ## step 2 : loop over all detections
        x_= bev_y_ /configs.bev_height*(configs.lim_x[1]-configs.lim_x[0])
        l_= bev_l_ /configs.bev_height*(configs.lim_x[1]-configs.lim_x[0])
        w_= bev_w_ /configs.bev_width*(configs.lim_y[1]-configs.lim_y[0])
        y_= bev_x_ /configs.bev_width*(configs.lim_y[1]-configs.lim_y[0]) -(configs.lim_y[1]-configs.lim_y[0])/2.0
            ## step 3 : perform the conversion using the limits for x, y and z set in the configs structure
        if((z_>=configs.lim_z[0]) and (z_<=configs.lim_z[1]) and (y_>=configs.lim_y[0]) and (y_<=configs.lim_y[1]) and (x_>=configs.lim_x[0]) and (x_<=configs.lim_x[1])):
            ## step 4 : append the current object to the 'objects' array
            objects.append([1, x_, y_, z_, h_, w_, l_, yaw_])
    #######
    ####### ID_S3_EX2 START #######   
